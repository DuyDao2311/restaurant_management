...
@router.get("")
def api_staff_get_reservations(
    ...
    """
    Staff xem danh sách Reservation của nhà hàng.
...
def api_staff_get_reservation_detail(
    reservation_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin_or_staff)
):
...
@router.patch("/{reservation_id}/assign-table")
def api_staff_assign_table(
    reservation_id: int,
...
def api_staff_reject_reservation(
    reservation_id: int,
...
def api_staff_checkin_reservation(
    reservation_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin_or_staff)
):
    """
    Staff Check-in cho Reservation.
...
def api_staff_get_reservation_session(
    reservation_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin_or_staff)
):
    """
    Staff lấy Session của một Reservation.
    """
